...
async def create_comment_content(comment: CommentCreate, db: AsyncSession):
    ...
    댓글 생성
    """
    print(comment.comment_name, comment.comment_content)
...
async def delete_comment_data(comment_name: str, comment_password: str, db: AsyncSession):
    """
    댓글 삭제
    """
    # 댓글 존재 확인
    comment_query = await db.execute(select(BlogComment).where(BlogComment.comment_name == comment_name))
